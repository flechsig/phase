...
#define IDL_V_FILE          8	/* An ASSOC variable. V_ARR is always set
				   when V_FILE is set (there are no scalar
				   ASSOC variables). */
...
#if defined(LINUX_ALPHA) || defined(HPUX_64)
...
#if (defined(unix) && !defined(hpux) && !defined(AIX))
...
#if defined(ALPHA_OSF) || defined(irix) || defined(WIN32)
/*
...
extern int IDL_CDECL IDL_KWProcessByOffset(int argc, IDL_VPTR *argv, char
        *argk, IDL_KW_PAR *kw_list, IDL_VPTR *plain_args,  int mask, void
        *base);
...
extern char *IDL_CDECL IDL_GetScratchOnThreshold(IDL_REGISTER char
        *auto_buf, IDL_REGISTER IDL_MEMINT auto_elts,  IDL_REGISTER
        IDL_MEMINT n_elts,  IDL_REGISTER IDL_MEMINT elt_size,  IDL_VPTR
        *tempvar);
...
extern void IDL_CDECL IDL_RasterDrawThick(IDL_GR_PT *p0, IDL_GR_PT *p1,
        IDL_ATTR_STRUCT *a,  IDL_DEVCORE_FCN_POLYFILL routine,  int
        dot_width);
...
extern void IDL_CDECL IDL_Raster8Image(UCHAR *data, IDL_ULONG nx,
        IDL_ULONG ny,  IDL_ULONG x0, IDL_ULONG y0,  IDL_ULONG xsize,
        IDL_ULONG ysize,  IDL_TV_STRUCT *secondary,  IDL_RASTER_DEF *rs, 
        IDLBool_t bReverse);
extern void IDL_CDECL IDL_RasterImage(UCHAR *data, IDL_ULONG nx, IDL_ULONG
        ny,  IDL_ULONG x0, IDL_ULONG y0,  IDL_ULONG xsize, IDL_ULONG
...
extern IDL_LONG IDL_CDECL IDL_grMesh_Clip(float fPlane[4], short clipSide,
         float *pfVin, IDL_LONG iNVerts,  IDL_LONG *piCin, IDL_LONG
        iNConn,  float **pfVout, IDL_LONG *iNVout,  IDL_LONG **piCout,
        IDL_LONG *iNCout,  IDL_VPTR vpAuxInKW, IDL_VPTR vpAuxOutKW, 
        IDL_VPTR vpCut);
...
extern int IDL_CDECL IDL_FileOpen(int argc, IDL_VPTR argv[], char *argk,
        int access_mode, IDL_SFILE_FLAGS_T extra_flags,  int longjmp_safe,
        int msg_attr);
...
extern int IDL_CDECL IDL_RuntimeVMExec(char *file);
