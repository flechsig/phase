 ...
 ; Time-stamp: <24 Jun 14 10:33:25 flechsig> 
...
result[0,*]=unwrap_phase_1d(refcol)
;; now correct all rows
FOR count = 0,(size(result))[2]-1 DO BEGIN
    refrow = result[*,count]
    result[*,count]=unwrap_phase_1d(refrow)
ENDFOR
...
FUNCTION myunwrap_phase, data
;+
; NAME:
;      MYUNWRAP_PHASE
;
; PURPOSE:
;      myunwrap phase jumps in one and two-dimensional arrays
;
; EXPLANATION:
...
;;    help, result
;;    print,'return from unwrap_phase'
    return, result
ENDIF ELSE BEGIN
    ;; find the phase jump locations
    jumps = data - shift(data,1)
