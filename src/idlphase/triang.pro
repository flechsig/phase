...
;
;
pro triang, dz=dz, dy=dy, Nz=Nz, Ny=Ny, sizez=sizez, sizey=sizey,  bcomp=bcomp, z_vec=z_vec, y_vec=y_vec, wavelength=wavelength, plot=plot
;+
; NAME:
;  slit  
; 
;  
; PURPOSE:
; Creates a triangle shaped amplitude distribution   with width   dz, dy   
;
;
; CATEGORY:
;
...
for i=0, Nz-1 do begin
  for j=0, Ny-1 do begin
     if ( (abs(z_vec[i]) le dzh) AND (abs(y_vec[j]) le dyh)) then amp = (1-abs(z_vec[i]/dzh)) *  (1-abs(y_vec[j]/dyh))  else  amp = 0
