...
;  Time-stamp: <25 Jan 08 13:51:34 flechsig> 
...
;  
;      !the script should not be installed with phase !
;      
...
;
 ...
 PRINTF,final_file,'<!-- This file was generated by my_mk_html_help.pro -->'
 ...
 PRINTF,final_file,'<P>'
 PRINTF,final_file,'This page was created by the the modified IDL library routine '
 PRINTF,final_file,'<CODE>my_mk_html_help</CODE>.  For more information on '
...
;;UF    PRINTF,final_file,'<STRONG>(See '+fname+')</STRONG><P>'
;;   PRINTF,final_file,'<STRONG>(See <A
    ;;   HREF=file:'+fname+'>'+fname+'</A>)</STRONG><P>'
;; reformat fname
     mypos= strpos(fname, '/', /reverse_search)
     myname= strmid(fname, mypos+1)
     PRINTF,final_file,'<STRONG>(See <A HREF='+myname+'>'+myname+'</A>)</STRONG><P>'
   ENDELSE
   PRINTF,final_file,'<HR>'
   PRINTF,final_file,' '
  ENDIF
...
;----------------------------------------------------------------------------
PRO my_mk_html_help, sources, outfile, VERBOSE=verbose, TITLE=title, STRICT=strict
