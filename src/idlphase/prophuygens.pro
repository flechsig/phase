...
pro  prophuygens, drift=drift, y_vec=y_vec, z_vec=z_vec, field=field $
                 ,Nu=Nu, Nv=Nv $
...
;   15.8. 13 renamed to prophuygens.pro                            
...
print, 'prophuygens start calculation  ---  drift=',drift
r   = double(1.0)                 
amp = double(1.0)                 
ppi = double(2 * !dpi)             
Nz  = n_elements(z_vec)           
...
;;          amp         = -1.0 * drift / r^2 / wavelength            ;; cos(n r)  = drift /r        
            amp         =        drift / r^2 / wavelength            ;; 16.8.2013       
